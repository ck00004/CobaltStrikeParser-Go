...
// 处理url中的/
// 组成GET stage HTTP/1.1
func Parse1(rawURL string) (*URL, error) {
	// Cut off #frag
	u, frag := split(rawURL, '#', true)
	url, err := parse1(u, false)
	if err != nil {
		return nil, &Error{"parse", u, err}
	}
	if frag == "" {
...
func parse1(rawURL string, viaRequest bool) (*URL, error) {
	var rest string
	var err error
	...
	// the default escaping of Path is equivalent, to help make sure that people
	// don't rely on it in general.
	if err := url.setPath1(rest); err != nil {
		return nil, err
	}
	return url, nil
}
...
func (u *URL) setPath1(p string) error {
	p = p[1:] //去除'\'
	path, err := unescape(p, encodePath)
	if err != nil {
		return err
